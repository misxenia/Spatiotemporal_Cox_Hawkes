...
# python run_output_analysis_goodness_of_fit.py --dataset_name 'LGCP_Hawkes' --simulation_number 0 --n_pred 200 --model_name 'LGCP_Hawkes' --simulate_predictions 'False' --calculate_error 'False' 
...
if __name__=='__main__':
	...
	print('mcmc keys',mcmc_samples.keys())
	...
	print('simulated_output_Hawkes_train_test keys', simulated_output_Hawkes_train_test.keys())
	
	print(simulated_output_Hawkes_train_test['index_array_train'].shape)
	print(simulated_output_Hawkes_train_test['G_tot_t_train'].shape)
	print(simulated_output_Hawkes['G_tot_t'].shape)
...
#
	...
	if args_train['background'] not in ['constant', 'Poisson']:
		...
		#spatial_background_integral=mcmc_samples['Itot_xy']#mcmc['Itot_xy]
		#print(spatial_background_integral.mean())
		spatial_background_integral=jnp.mean(predictions['Itot_xy'], axis=0)#mcmc['Itot_xy]
		print(spatial_background_integral)
	...
	if args['background'] in ['LGCP_Hawkes', 'LGCP_only']:
		### PLOT
		#ft_true=np.zeros(args_train['n_t'])
		#f_xy_true=np.zeros(args_train['n_xy']**2)
		indices_t=simulated_output_Hawkes_background['ind_t_i']
		indices_xy=simulated_output_Hawkes_background['ind_xy_i']
	...
	if model_name in ['LGCP','LGCP_Hawkes']:
	...
	if args_train['background'] not in ['LGCP_only','Poisson']:
		...
		alpha_post_mean=alpha_post_samples.mean()
		...
		gaussian_integral_post_mean=np.array(mcmc_samples['gaussianpart'][n_total-post_samples:n_total].mean())
		gaussian_integral_post_samples=np.array(mcmc_samples['gaussianpart'][n_total-post_samples:n_total])
		print('gaussian_integral_post_mean',gaussian_integral_post_samples.mean())
	...
	event_times=
	const_A=spatial_background_integral
	const_B=gaussian_integral_post_mean
	mu_0_post_mean=np.exp(a_0_post_mean)
	
	T=args['T']
	#ind_back_t=simulated_output_background['ind_t_i'][0][sorted_ind]
	back_t=args['x_t'][ind_back_t]
	mean_f=f_t_pred_mean
	indices=indices_t

  _,estimated_lambda_inten,estim_A_new=lambda_t_calculation_M5(mu_0_post_mean, alpha_post_mean, beta_post_mean, T, back_t, mean_f[indices],const_A, const_B, event_times)
	#numerical way to compute the cdf ie the integral
	N=
	integ_inten=[np.trapz(estimated_lambda_inten[i:i+2],event_times[i:i+2]) for i in range(0,N-2)]
	#integ_inten2=[lambda_inten[i]*(event_times[i+1]-event_times[i]) for i in range(N-1)]
	#QQ plot
	fig = plt.figure()
	ax = fig.add_subplot(1)
	res = stats.probplot(integ_inten, dist="expon", plot=pylab, plot=ax)
	...
	PARAMS=pd.DataFrame()
	PARAMS=pd.DataFrame({'a_0': np.zeros(args['n_xy']**2),'beta': np.zeros(args['n_xy']**2),'alpha': np.zeros(args['n_xy']**2), 'sigma_x_2': np.zeros(args['n_xy']**2), 'f_t':np.zeros(args['n_xy']**2), 'f_xy':np.zeros(args['n_xy']**2)} )
	...
	PARAMS['gaussian_integral']=gaussian_integral_post_mean
	PARAMS['spatial_background_integral']=spatial_background_integral
	
	print('f_t_pred_mean',f_t_pred_mean.shape)
	print('args[T]',args['T'])
